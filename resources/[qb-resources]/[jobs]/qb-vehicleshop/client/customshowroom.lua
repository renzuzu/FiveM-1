                ...
                if IsPedInAnyVehicle(ped, false) then
                    if dist < 15 then
                        ...
                        if dist < 2 then
                            ...
                            QBCore.Functions.ShowFloatingHelpNotification("[~g~E~w~] Return the vehicle",vector3(bringcoords.x,bringcoords.y,bringcoords.z))
                            ...
                            if QBCustom.ShowroomPositions[ClosestCustomVehicle] ~= nil then
                                if QBCustom.ShowroomPositions[ClosestCustomVehicle].buying then
                                    if QBCore.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle] ~= nil then
                                        ...
                                        QBCore.Functions.ShowFloatingHelpNotification("~g~7~w~ - Buy / ~r~8~w~ - Cancel \n".."~g~$"..QBCore.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle].price,vector3(QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.x, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.y, QBCustom.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.6))
                                        if IsDisabledControlJustPressed(0, Keys["7"]) then
                                            ...
                                            elseif finance then
                                                TriggerServerEvent('qb-vehicleshop:server:ConfirmVehicleFinance', QBCustom.ShowroomPositions[ClosestCustomVehicle])
                                                QBCustom.ShowroomPositions[ClosestCustomVehicle].buying = false
                                            end
                                        end
...
RegisterNetEvent('qb-vehicleshop:client:FinanceCustomVehicle')
AddEventHandler('qb-vehicleshop:client:FinanceCustomVehicle', function(TargetId)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local player, distance = GetClosestPlayer()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if VehicleDist < 2.5 then
            TriggerServerEvent('qb-vehicleshop:server:FinanceCustomVehicle', TargetId, ClosestCustomVehicle)
        else
            QBCore.Functions.Notify("You are not near the vehicle!", "error")
        end
    else
...
RegisterNetEvent('qb-vehicleshop:client:SetVehicleFinance')
AddEventHandler('qb-vehicleshop:client:SetVehicleFinance', function(slot)
    QBCustom.ShowroomPositions[slot].buying = true
    finance = true
    SetTimeout((60 * 1000) * 5, function()
        QBCustom.ShowroomPositions[slot].buying = false
